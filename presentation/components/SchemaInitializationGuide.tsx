...
const SchemaInitializationGuide: React.FC<SchemaInitializationGuideProps> = ({ onRetry }) => {
  ...
  const setupSQL = `
-- Danum POS - Full Database Setup v20 (Editable Purchase Date)
-- This script is idempotent and can be run multiple times safely.
-- It creates the complete schema and brings it to the latest version.
...
CREATE TABLE IF NOT EXISTS public.products (id bigint PRIMARY KEY, name text NOT NULL, price numeric NOT NULL, image_url text NOT NULL, is_for_sale boolean NOT NULL DEFAULT true, stock_level numeric NULL, stock_unit text NULL);
...
CREATE TABLE IF NOT EXISTS public.product_recipe_items (id bigserial PRIMARY KEY, product_id bigint NOT NULL REFERENCES public.products(id) ON DELETE CASCADE, ingredient_id bigint NULL REFERENCES public.ingredients(id) ON DELETE CASCADE, sub_product_id bigint NULL REFERENCES public.products(id) ON DELETE CASCADE, quantity numeric NOT NULL, unit text NOT NULL, CONSTRAINT one_recipe_item_type_check CHECK ((ingredient_id IS NOT NULL AND sub_product_id IS NULL) OR (ingredient_id IS NULL AND sub_product_id IS NOT NULL)), UNIQUE(product_id, ingredient_id), UNIQUE(product_id, sub_product_id));
CREATE TABLE IF NOT EXISTS public.open_orders (id uuid PRIMARY KEY DEFAULT gen_random_uuid(), table_number text NOT NULL UNIQUE, order_data jsonb NOT NULL, user_id uuid NOT NULL REFERENCES public.users(id), created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now());
CREATE TABLE IF NOT EXISTS public.purchase_log (id bigserial PRIMARY KEY, created_at timestamptz NOT NULL DEFAULT now(), ingredient_id bigint NOT NULL REFERENCES public.ingredients(id), quantity_purchased numeric NOT NULL, unit text NOT NULL, total_cost numeric NOT NULL, user_id uuid NULL REFERENCES public.users(id), supplier text NULL, notes text NULL);
...
CREATE TRIGGER set_open_orders_updated_at BEFORE UPDATE ON public.open_orders FOR EACH ROW EXECUTE FUNCTION public.set_current_timestamp_updated_at();

-- Section 4: Product, Category & Inventory Functions
...
CREATE OR REPLACE FUNCTION public.get_products_with_categories() RETURNS TABLE(id bigint, name text, price numeric, image_url text, categories text[], recipe jsonb, is_for_sale boolean, stock_level numeric, stock_unit text) LANGUAGE sql STABLE SECURITY DEFINER AS $$ SELECT p.id, p.name, p.price, p.image_url, COALESCE((SELECT array_agg(c.name) FROM public.product_categories pc JOIN public.categories c ON pc.category_id = c.id WHERE pc.product_id = p.id), '{}'::text[]) as categories, COALESCE( (SELECT jsonb_agg( CASE WHEN pri.ingredient_id IS NOT NULL THEN jsonb_build_object( 'type', 'ingredient', 'ingredientId', pri.ingredient_id, 'ingredientName', i.name, 'quantity', pri.quantity, 'unit', pri.unit ) WHEN pri.sub_product_id IS NOT NULL THEN jsonb_build_object( 'type', 'product', 'productId', pri.sub_product_id, 'productName', sub_p.name, 'quantity', pri.quantity, 'unit', pri.unit ) END ) FROM public.product_recipe_items pri LEFT JOIN public.ingredients i ON pri.ingredient_id = i.id LEFT JOIN public.products sub_p ON pri.sub_product_id = sub_p.id WHERE pri.product_id = p.id), '[]'::jsonb ) as recipe, p.is_for_sale, p.stock_level, p.stock_unit FROM public.products p ORDER BY p.name; $$;
...
CREATE OR REPLACE FUNCTION public.get_all_conversions() RETURNS TABLE(id bigint, from_unit text, to_unit text, factor numeric, ingredient_id bigint, ingredient_name text) LANGUAGE sql STABLE SECURITY DEFINER AS $$ SELECT uc.id, uc.from_unit, uc.to_unit, uc.factor, uc.ingredient_id, i.name as ingredient_name FROM public.unit_conversions uc LEFT JOIN public.ingredients i ON uc.ingredient_id = i.id ORDER BY i.name, uc.from_unit; $$;
CREATE OR REPLACE FUNCTION public.create_conversion(p_from_unit text, p_to_unit text, p_factor numeric, p_ingredient_id bigint DEFAULT NULL) RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $$ BEGIN INSERT INTO public.unit_conversions (from_unit, to_unit, factor, ingredient_id) VALUES (p_from_unit, p_to_unit, p_factor, p_ingredient_id); END; $$;
CREATE OR REPLACE FUNCTION public.update_conversion(p_id bigint, p_from_unit text, p_to_unit text, p_factor numeric, p_ingredient_id bigint DEFAULT NULL) RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $$ BEGIN UPDATE public.unit_conversions SET from_unit = p_from_unit, to_unit = p_to_unit, factor = p_factor, ingredient_id = p_ingredient_id WHERE id = p_id; END; $$;
CREATE OR REPLACE FUNCTION public.delete_conversion(p_id bigint) RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $$ BEGIN DELETE FROM public.unit_conversions WHERE id = p_id; END; $$;

-- Section 5: Sales, Seeding & Conversion Functions
CREATE OR REPLACE FUNCTION public.get_conversion_factor(p_from_unit text, p_to_unit text, p_ingredient_id bigint) RETURNS numeric LANGUAGE plpgsql SECURITY DEFINER AS $$ DECLARE v_factor numeric; BEGIN IF p_from_unit = p_to_unit THEN RETURN 1.0; END IF; WITH RECURSIVE all_possible_edges AS ( SELECT from_unit, to_unit, factor, ingredient_id FROM public.unit_conversions WHERE ingredient_id = p_ingredient_id OR ingredient_id IS NULL UNION ALL SELECT to_unit as from_unit, from_unit as to_unit, 1.0/factor as factor, ingredient_id FROM public.unit_conversions WHERE ingredient_id = p_ingredient_id OR ingredient_id IS NULL ), distinct_edges AS ( SELECT DISTINCT ON (from_unit, to_unit) from_unit, to_unit, factor FROM all_possible_edges ORDER BY from_unit, to_unit, ingredient_id DESC NULLS LAST ), conversion_path AS ( SELECT p_from_unit as end_unit, 1.0::numeric as total_factor, ARRAY[p_from_unit] as path_array, 1 as depth UNION ALL SELECT de.to_unit as end_unit, cp.total_factor * de.factor, cp.path_array || de.to_unit, cp.depth + 1 FROM conversion_path cp JOIN distinct_edges de ON cp.end_unit = de.from_unit WHERE NOT (de.to_unit = ANY(cp.path_array)) AND cp.depth < 10 ) SELECT total_factor INTO v_factor FROM conversion_path WHERE end_unit = p_to_unit ORDER BY depth ASC LIMIT 1; IF v_factor IS NOT NULL THEN RETURN v_factor; END IF; RAISE EXCEPTION 'No conversion path found from % to % for ingredient %', p_from_unit, p_to_unit, p_ingredient_id; END; $$;
CREATE OR REPLACE FUNCTION public.log_purchase(p_ingredient_id bigint, p_quantity numeric, p_unit text, p_total_cost numeric, p_user_id uuid, p_supplier text DEFAULT NULL, p_notes text DEFAULT NULL, p_created_at timestamptz DEFAULT now()) RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $$ DECLARE v_stock_unit text; v_conversion_factor numeric; v_stock_to_add numeric; BEGIN INSERT INTO public.purchase_log (created_at, ingredient_id, quantity_purchased, unit, total_cost, user_id, supplier, notes) VALUES (p_created_at, p_ingredient_id, p_quantity, p_unit, p_total_cost, p_user_id, p_supplier, p_notes); SELECT stock_unit INTO v_stock_unit FROM public.ingredients WHERE id = p_ingredient_id; v_conversion_factor := public.get_conversion_factor(p_unit, v_stock_unit, p_ingredient_id); v_stock_to_add := p_quantity * v_conversion_factor; UPDATE public.ingredients SET stock_level = stock_level + v_stock_to_add WHERE id = p_ingredient_id; END; $$;
CREATE OR REPLACE FUNCTION public.get_purchase_log(p_end_date date, p_start_date date) RETURNS TABLE(id bigint, created_at timestamptz, total_cost numeric, cashier_username text, ingredient_name text, quantity_purchased numeric, unit text, supplier text, notes text) LANGUAGE sql STABLE SECURITY DEFINER AS $$ SELECT pl.id, pl.created_at, pl.total_cost, u.username as cashier_username, i.name as ingredient_name, pl.quantity_purchased, pl.unit, pl.supplier, pl.notes FROM public.purchase_log pl LEFT JOIN public.users u ON pl.user_id = u.id JOIN public.ingredients i ON pl.ingredient_id = i.id WHERE pl.created_at >= p_start_date AND pl.created_at < p_end_date + interval '1 day' ORDER BY pl.created_at DESC; $$;
CREATE OR REPLACE FUNCTION public.restock_preparation(p_product_id bigint, p_quantity_to_add numeric) RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $$ DECLARE recipe_item RECORD; v_conversion_factor numeric; v_deduction_amount numeric; v_stock_unit text; BEGIN UPDATE public.products SET stock_level = stock_level + p_quantity_to_add WHERE id = p_product_id; FOR recipe_item IN SELECT pri.ingredient_id, pri.quantity, pri.unit FROM public.product_recipe_items pri WHERE pri.product_id = p_product_id AND pri.ingredient_id IS NOT NULL LOOP SELECT stock_unit INTO v_stock_unit FROM public.ingredients WHERE id = recipe_item.ingredient_id; v_conversion_factor := public.get_conversion_factor(recipe_item.unit, v_stock_unit, recipe_item.ingredient_id); v_deduction_amount := recipe_item.quantity * p_quantity_to_add * v_conversion_factor; UPDATE public.ingredients SET stock_level = stock_level - v_deduction_amount WHERE id = recipe_item.ingredient_id; END LOOP; END; $$;
DROP FUNCTION IF EXISTS public.create_order(numeric, jsonb, uuid);
CREATE OR REPLACE FUNCTION public.create_order(p_total numeric, p_items jsonb, p_user_id uuid) RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $$ DECLARE v_order_id uuid; order_item jsonb; v_product_deduction RECORD; v_ingredient_deduction RECORD; v_conversion_factor numeric; v_deduction_amount numeric; v_stock_unit text; BEGIN INSERT INTO public.orders (total, user_id) VALUES (p_total, p_user_id) RETURNING id INTO v_order_id; FOR order_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP INSERT INTO public.order_items (order_id, product_id, quantity, price) VALUES (v_order_id, (order_item->>'product_id')::bigint, (order_item->>'quantity')::integer, (order_item->>'price')::numeric); END LOOP; FOR v_product_deduction IN WITH RECURSIVE bom(product_id, quantity, level) AS ( SELECT (item->>'product_id')::bigint, (item->>'quantity')::numeric, 0 FROM jsonb_array_elements(p_items) as item UNION ALL SELECT pri.sub_product_id, bom.quantity * pri.quantity, bom.level + 1 FROM bom JOIN public.products p ON p.id = bom.product_id JOIN public.product_recipe_items pri ON pri.product_id = p.id WHERE p.stock_level IS NULL AND pri.sub_product_id IS NOT NULL AND bom.level < 10 ) SELECT b.product_id, SUM(b.quantity) as total_quantity FROM bom b JOIN public.products p ON p.id = b.product_id WHERE p.stock_level IS NOT NULL GROUP BY b.product_id LOOP UPDATE public.products SET stock_level = stock_level - v_product_deduction.total_quantity WHERE id = v_product_deduction.product_id; END LOOP; FOR v_ingredient_deduction IN WITH RECURSIVE bom(product_id, quantity, level) AS ( SELECT (item->>'product_id')::bigint, (item->>'quantity')::numeric, 0 FROM jsonb_array_elements(p_items) as item UNION ALL SELECT pri.sub_product_id, bom.quantity * pri.quantity, bom.level + 1 FROM bom JOIN public.products p ON p.id = bom.product_id JOIN public.product_recipe_items pri ON pri.product_id = p.id WHERE p.stock_level IS NULL AND pri.sub_product_id IS NOT NULL AND bom.level < 10 ) SELECT pri.ingredient_id, pri.unit, SUM(bom.quantity * pri.quantity) as total_quantity FROM bom JOIN public.products p ON p.id = bom.product_id JOIN public.product_recipe_items pri ON pri.product_id = p.id WHERE p.stock_level IS NULL AND pri.ingredient_id IS NOT NULL GROUP BY pri.ingredient_id, pri.unit LOOP SELECT stock_unit INTO v_stock_unit FROM public.ingredients WHERE id = v_ingredient_deduction.ingredient_id; v_conversion_factor := public.get_conversion_factor(v_ingredient_deduction.unit, v_stock_unit, v_ingredient_deduction.ingredient_id); v_deduction_amount := v_ingredient_deduction.total_quantity * v_conversion_factor; UPDATE public.ingredients SET stock_level = stock_level - v_deduction_amount WHERE id = v_ingredient_deduction.ingredient_id; END LOOP; END; $$;
CREATE OR REPLACE FUNCTION public.get_sales_report(p_start_date date, p_end_date date) RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER AS $$ DECLARE report jsonb; v_total_revenue numeric; v_total_expenses numeric; BEGIN SELECT COALESCE(SUM(o.total), 0), jsonb_build_object( 'orderCount', COUNT(o.id), 'avgOrderValue', COALESCE(AVG(o.total), 0), 'dailySales', (SELECT COALESCE(jsonb_agg(ds), '[]'::jsonb) FROM (SELECT DATE(o2.created_at) AS date, SUM(o2.total) AS total FROM public.orders o2 WHERE o2.created_at >= p_start_date AND o2.created_at < p_end_date + interval '1 day' GROUP BY DATE(o2.created_at) ORDER BY date) ds), 'topProducts', (SELECT COALESCE(jsonb_agg(tp), '[]'::jsonb) FROM (SELECT p.name AS name, p.price AS price, SUM(oi.quantity) AS quantity FROM public.order_items oi JOIN public.products p ON oi.product_id = p.id JOIN public.orders o3 ON oi.order_id = o3.id WHERE o3.created_at >= p_start_date AND o3.created_at < p_end_date + interval '1 day' GROUP BY p.name, p.price ORDER BY quantity DESC LIMIT 10) tp) ) INTO v_total_revenue, report FROM public.orders o WHERE o.created_at >= p_start_date AND o.created_at < p_end_date + interval '1 day';
SELECT COALESCE(SUM(pl.total_cost), 0) INTO v_total_expenses FROM public.purchase_log pl WHERE pl.created_at >= p_start_date AND pl.created_at < p_end_date + interval '1 day';
report := report || jsonb_build_object( 'totalRevenue', v_total_revenue, 'totalExpenses', v_total_expenses, 'netProfit', v_total_revenue - v_total_expenses, 'dailyExpenses', ( SELECT COALESCE(jsonb_agg(de), '[]'::jsonb) FROM ( SELECT DATE(pl2.created_at) AS date, SUM(pl2.total_cost) AS total FROM public.purchase_log pl2 WHERE pl2.created_at >= p_start_date AND pl2.created_at < p_end_date + interval '1 day' GROUP BY DATE(pl2.created_at) ORDER BY date ) de ) ); RETURN report; END; $$;
...
CREATE OR REPLACE FUNCTION public.get_all_open_orders() RETURNS TABLE(table_number text, order_data jsonb) LANGUAGE sql STABLE SECURITY DEFINER AS $$ SELECT o.table_number, o.order_data FROM public.open_orders o ORDER BY o.table_number; $$;
CREATE OR REPLACE FUNCTION public.save_open_order(p_order_data jsonb, p_table_number text, p_user_id uuid) RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $$ BEGIN INSERT INTO public.open_orders (table_number, order_data, user_id) VALUES (p_table_number, p_order_data, p_user_id) ON CONFLICT (table_number) DO UPDATE SET order_data = EXCLUDED.order_data, user_id = EXCLUDED.user_id, updated_at = now(); END; $$;
CREATE OR REPLACE FUNCTION public.close_open_order(p_table_number text) RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $$ BEGIN DELETE FROM public.open_orders WHERE table_number = p_table_number; END; $$;

-- Section 6. User & Auth Functions
...
CREATE POLICY "Allow full access to open orders" ON public.open_orders FOR ALL USING (true) WITH CHECK(true);
CREATE POLICY "Allow full access to purchase log" ON public.purchase_log FOR ALL USING (true) WITH CHECK(true);
  ...
  return (
    <div className="bg-surface-card rounded-xl shadow-2xl p-8 w-full max-w-4xl mx-4 animate-fade-in">
      ...
      <div className="space-y-6">
        ...
        <div>
          <h2 className="text-lg font-semibold text-text-primary mb-2">Complete Setup Script (v20)</h2>
