...
const SchemaInitializationGuide: React.FC<SchemaInitializationGuideProps> = ({ onRetry }) => {
  ...
  const setupSQL = `
-- Danum POS - Full Database Setup v17 (Open Orders Fix)
-- This script is idempotent and can be run multiple times safely.
-- It creates the complete schema and brings it to the latest version.

-- Section 1: Core Tables (Unchanged)
...
CREATE TABLE IF NOT EXISTS public.product_recipe_items (
    id bigserial PRIMARY KEY, product_id bigint NOT NULL REFERENCES public.products(id) ON DELETE CASCADE, ingredient_id bigint NULL REFERENCES public.ingredients(id) ON DELETE CASCADE, sub_product_id bigint NULL REFERENCES public.products(id) ON DELETE CASCADE,
    quantity numeric NOT NULL, unit text NOT NULL, CONSTRAINT one_recipe_item_type_check CHECK ((ingredient_id IS NOT NULL AND sub_product_id IS NULL) OR (ingredient_id IS NULL AND sub_product_id IS NOT NULL)),
    UNIQUE(product_id, ingredient_id), UNIQUE(product_id, sub_product_id)
);
CREATE TABLE IF NOT EXISTS public.open_orders (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(), table_number text NOT NULL UNIQUE, order_data jsonb NOT NULL, user_id uuid NOT NULL REFERENCES public.users(id),
  ...
  return (
    <div className="bg-surface-card rounded-xl shadow-2xl p-8 w-full max-w-4xl mx-4 animate-fade-in">
      ...
      <div className="space-y-6">
        ...
        <div>
          <h2 className="text-lg font-semibold text-text-primary mb-2">Complete Setup Script (v17)</h2>
