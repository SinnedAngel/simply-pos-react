...
const SchemaInitializationGuide: React.FC<SchemaInitializationGuideProps> = ({ onRetry }) => {
  ...
  const setupSQL = `
-- Danum POS - Full Database Setup v14 (Restock Logic)
-- This script is idempotent and can be run multiple times safely.
-- It adds a new function, restock_preparation, to automatically handle ingredient deduction.
...
-- Section 2: Product Catalog, Sales, & Inventory Tables (Unchanged from v13)
...
-- Section 3: Product, Category & Inventory Functions (Unchanged from v13)
...
BEGIN
    ...
    LOOP
        INSERT INTO public.products(id, name, price, image_url, is_for_sale, stock_level, stock_unit)
        VALUES (
...
-- Section 4: Sales, Seeding & Conversion Functions (UPDATED)
...
BEGIN
    IF p_from_unit = p_to_unit THEN RETURN 1.0; END IF;
    ...
    distinct_edges AS (
        SELECT DISTINCT ON (from_unit, to_unit) from_unit, to_unit, factor FROM all_possible_edges ORDER BY from_unit, to_unit, ingredient_id DESC NULLS LAST
    ),
    conversion_path AS (
        SELECT p_from_unit as end_unit, 1.0::numeric as total_factor, ARRAY[p_from_unit] as path_array, 1 as depth
        UNION ALL
        SELECT de.to_unit as end_unit, cp.total_factor * de.factor, cp.path_array || de.to_unit, cp.depth + 1
        FROM conversion_path cp JOIN distinct_edges de ON cp.end_unit = de.from_unit
        WHERE NOT (de.to_unit = ANY(cp.path_array)) AND cp.depth < 10
    )
    SELECT total_factor INTO v_factor FROM conversion_path WHERE end_unit = p_to_unit ORDER BY depth ASC LIMIT 1;
    IF v_factor IS NOT NULL THEN RETURN v_factor; END IF;
    RAISE EXCEPTION 'No conversion path found from % to % for ingredient %', p_from_unit, p_to_unit, p_ingredient_id;
END;
$$;
...
DECLARE
    v_order_id uuid;
    order_item jsonb;
    v_product_deduction RECORD;
    v_ingredient_deduction RECORD;
    v_conversion_factor numeric;
    v_deduction_amount numeric;
    v_stock_unit text;
BEGIN
    ...
    FOR v_product_deduction IN
        ...
        SELECT b.product_id, SUM(b.quantity) as total_quantity
        FROM bom b
        JOIN public.products p ON p.id = b.product_id
        WHERE p.stock_level IS NOT NULL
    ...
    FOR v_ingredient_deduction IN
        ...
        JOIN public.product_recipe_items pri ON pri.product_id = p.id
...
-- Section 5. User & Auth Functions (Unchanged)
...
-- Section 6. Seed Initial Data (Unchanged)
DO $$
...
-- Section 7. Policies and Permissions (Unchanged)
...
-- Section 8. Storage Bucket (Unchanged)
  ...
  return (
    <div className="bg-surface-card rounded-xl shadow-2xl p-8 w-full max-w-4xl mx-4 animate-fade-in">
      ...
      <div className="space-y-6">
        ...
        <div>
          <h2 className="text-lg font-semibold text-text-primary mb-2">Complete Setup Script (v14)</h2>
